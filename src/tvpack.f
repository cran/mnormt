...
*
      ...
      END
*
C     DOUBLE PRECISION FUNCTION STUDNT( NU, T )
*
...
C     IF ( NU .LT. 1 ) THEN
C        STUDNT = PHID( T )
C     ELSE IF ( NU .EQ. 1 ) THEN
C        STUDNT = ( 1 + 2*ATAN(T)/PI )/2
C     ELSE IF ( NU .EQ. 2 ) THEN
C        STUDNT = ( 1 + T/SQRT( 2 + T*T ))/2
C     ELSE 
C        TT = T*T
C        CSSTHE = 1/( 1 + TT/NU )
C        POLYN = 1
C        DO J = NU-2, 2, -2
C           POLYN = 1 + ( J - 1 )*CSSTHE*POLYN/J
C        END DO
C        IF ( MOD( NU, 2 ) .EQ. 1 ) THEN
C           RN = NU
C           TS = T/SQRT(RN)
C           STUDNT = ( 1 + 2*( ATAN(TS) + TS*CSSTHE*POLYN )/PI )/2
C        ELSE
C           SNTHE = T/SQRT( NU + TT )
C           STUDNT = ( 1 + SNTHE*POLYN )/2
C        END IF
C        STUDNT = MAX( ZRO, MIN( STUDNT, ONE ) )
C     ENDIF
C     END
*
